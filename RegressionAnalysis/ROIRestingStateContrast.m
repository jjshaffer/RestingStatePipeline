function x = ROIRestingStateContrast(covarFile, prefix, nullModel, model)
...
BOLDTable = table(covars.Subject, covars.Group, covars.Session, covars.Age, covars.Sex, covars.RDLPFCCBM, 'VariableNames', {'Subject', 'Group', 'Session', 'Age', 'Sex', 'BOLD'});
...
%j=i;
  ...
  %  for k = 1:zdim
        ...
        if(~isempty(model0) && ~isempty(model1))
            ...
            stats(1,11) = results.LRStat(2);
            stats(1,12) = results.pValue(2);
        end
        ...
        if(~isempty(model1))
            ...
            for l = 1:c
                if strcmp(model1.Coefficients{l,1}, 'Group:Session')
                    ...
                    stats(1,1) = model1.Coefficients.tStat(l);
                    stats(1,2) = model1.Coefficients.pValue(l);
                    %disp('ping');
                elseif strcmp(model1.Coefficients{l,1}, 'Group')
                    ...
                    stats(1,3) = model1.Coefficients.tStat(l);
                    stats(1,4) = model1.Coefficients.pValue(l);
                elseif strcmp(model1.Coefficients{l,1}, 'Session')
                    ...
                    stats(1,5) = model1.Coefficients.tStat(l);
                    stats(1,6) = model1.Coefficients.pValue(l);
                elseif strcmp(model1.Coefficients{l,1}, 'Age')
                    %disp(strcat(num2str(i), ':', num2str(j), ': ', num2str(k), ': ', num2str(model1.Coefficients.tStat(l))));
                    %%%stats(j,k,7) = model1.Coefficients.tStat(l);
                    %%%stats(j,k,8) = model1.Coefficients.pValue(l);
                    
                    stats(1,7) = model1.Coefficients.tStat(l);
                    stats(1,8) = model1.Coefficients.pValue(l);
                elseif strcmp(model1.Coefficients{l,1}, 'Sex')
                    ...
                    stats(1,9) = model1.Coefficients.tStat(l);
                    stats(1,10) = model1.Coefficients.pValue(l);
                end
            end %l loop
        end
    %end %k loop
